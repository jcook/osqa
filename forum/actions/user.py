...
class AwardPointsAction(ActionProxy):
    ...
    def repute_users(self):
        ...
        messages.success(REQUEST_HOLDER.request, _("Congratulations, you have been awarded an extra %(points)s reputation %(points_label)s on <a href=\"%(answer_url)s\">this</a> answer.") % {
                        'points': self._value,
...
class AwardAction(ActionProxy):
    ...
    def process_action(self):
        ...
        messages.success(REQUEST_HOLDER.request, _("""Congratulations, you have received a badge '%(badge_name)s'. <a href="%(badge_url)s">Find out who has it, too</a>.""") % dict(
...
class SuspendAction(ActionProxy):
    ...
    def cancel_action(self):
        for u in User.objects.filter(reputes__action=self).distinct():
            ...
            u.save()
            messages(REQUEST_HOLDER.request, _("Your suspension has been removed."))
